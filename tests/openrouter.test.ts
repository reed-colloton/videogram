...
const validResponse = () => ({
  choices: [
    { finish_reason: 'stop', message: { content: JSON.stringify(demo) } },
  ],
});
...
test('speech maps both UI voices to the selected speech model and MP3', () => {
  for (const voice of VOICE_OPTIONS) {
    ...
    assert.equal(request.model, 'minimax/speech-2.8-hd');
...
test('parses completed lessons and validates exact slide count', () => {
  assert.deepEqual(parseLessonResponse(validResponse(), 5), demo);
  assert.throws(
  ...
  for (const finish_reason of ['length', 'content_filter', 'error', null])
    assert.throws(() =>
      parseLessonResponse(
        {
          choices: [
            { finish_reason, message: { content: JSON.stringify(demo) } },
          ],
        },
        5,
      ),
